...
while True:
    if unload.value is True:
        for tube in range(len(load_tubes)):
    ...
    # check if any of the fire buttons is pressed
    nothingPressed = all(item is False for item in list((btn.is_pressed for btn in fire)))

    # release safe only if all fire buttons are off
    if safety.value is True and nothingPressed is True:
        safe = False
    
    # reset to save
    if safety.value is False:
        safe = True
